...
$pdo_search = $pdo_conn->prepare("SELECT * FROM user WHERE fullname LIKE '%$search_keyword%' OR studentnumber LIKE '%$search_keyword%' OR id LIKE '%$search_keyword%' OR username LIKE '%$search_keyword%' ORDER BY rand() limit 10");
...
while($row = $pdo_search->fetch()){?>
       ...
       <td><?php echo $row["time_date"];?></td><?php
}
}}
?>
